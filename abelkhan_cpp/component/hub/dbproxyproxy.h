...
public:
	dbproxyproxy(std::shared_ptr<juggle::Ichannel> ch) {
		_dbproxy_ch = ch;
		_dbproxy_caller = std::make_shared<caller::hub_call_dbproxy>(_dbproxy_ch);
	}
	...
	std::unordered_map<std::string, std::function<void()> > updata_persisted_object_callback;
	std::unordered_map<std::string, std::function<void(uint64_t)> > get_object_count_callback;
	std::unordered_map<std::string, std::function<void(Fossilizid::JsonParse::JsonArray)> > get_object_info_callback;
	std::unordered_map<std::string, std::function<void()> > get_object_info_end_callback;
	std::unordered_map<std::string, std::function<void()> > remove_object_callback;
