...
def gen_module_module(module_name, funcs):
        ...
        for i in funcs:
                ...
                if i[1] == "ntf":
                        ...
                        code += "    " + func_name + "(remote_hub_name:string, uuid:string"
                        count = 0
                        for item in i[2]:
                                code += ", argv" + str(count) + ":" + tools.genprototype(item)
                                count = count + 1
                        ...
                        code += "        this.hub_handle.modules.rsp = new rsp_" + func_name + "(this.hub_handle, remote_hub_name, uuid);\n"
                        ...
                        rsp_code += "    private remote_hub_name:string;\n"
                        rsp_code += "    private uuid:string;\n"
                        rsp_code += "    constructor(_hub:any, _remote_hub_name:string, _uuid:string){\n"
                        ...
                        for item in i[4]:
                                ...
                                count = count + 1
                        rsp_code += "        this.hub_handle.hubs.call_hub(this.remote_hub_name, \"cb_" + module_name + "\", \"" + func_name + "_rsp\", this.uuid"
                        count = 0
                        for item in i[4]:
                        ...
                        for item in i[6]:
                                rsp_code += tools.genlocaltoproto(item, count)
                                count = count + 1
                        rsp_code += "        this.hub_handle.hubs.call_hub(this.remote_hub_name, \"cb_" + module_name + "\", \"" + func_name + "_err\", this.uuid"
                        count = 0
                        for item in i[6]:
                                rsp_code += ", input_argv" + str(count)
                                count = count + 1
                        ...
                        raise Exception("func:%s wrong rpc type:%s must req or ntf" % (func_name, i[1]))
...
def genmodule(file_name, modules, enums, struct):
        ...
        module_code = ""
        for module_name, modlue in modules.items():
                module_code += gen_module_module(module_name, modlue["method"])
