...
class pycontainer(pyelement):
	...
	def remove(self):
		code = "    var obj = document.getElementById(\"" + self.id + "_1\");\n"
		code += "   document.body.removeChild(obj);"
	...
	def set_visibility(self, isvisibility):
		for div in self.divlist:
			div.set_visibility(isvisibility)

		super(pycontainer, self).set_visibility(isvisibility)

	def server_set_visible(self, isvisible):
		code = ""

		for div in self.divlist:
			code += div.server_set_visible(isvisible)

		code += super(pycontainer, self).server_set_visible(isvisible)
