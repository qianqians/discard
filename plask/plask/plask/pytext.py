...
class pytext(pyelement):
	...
	def server_set_text_key(self, key):
		return "document.getElementById(\"" + self.id + "_1\").innerHTML = value[\"" + key  + "\"];"

class pydynamictext(pyelement):
	def __init__(self, key, cname, layout, praframe):
		# when normal
		self.key = key
		self.type = "div"

		super(pydynamictext, self).__init__(cname, layout, praframe)
	...
	def sub(self, id = None):
		...
		js += super(pydynamictext, self).sub()
