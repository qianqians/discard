...
class pylink(pyelement):
	...
	def __init__(self, link, linkdescribe, cname, layout, praframe, embeddedext = None):
		...
		super(pylink, self).__init__(cname, layout, praframe)
...
class pydynamiclink(pylink):
	def __init__(self, key, cname, layout, praframe):
		# when normal
		self.key = key
		self.type = "a"

		super(pydynamiclink, self).__init__(cname, layout, praframe)
	...
	def sub(self, id = None):
		...
		js += super(pydynamiclink, self).sub()
