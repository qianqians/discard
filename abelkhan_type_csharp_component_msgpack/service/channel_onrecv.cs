...
namespace abelkhan
{
    public class channel_onrecv
    {
        ...
        private readonly Queue<ProtoRoot> que = new Queue<ProtoRoot>();

        public delegate void cb_recv_data(byte[] data);
        public event cb_recv_data on_recv_data;

        public ProtoRoot pop()
        {
            ProtoRoot _event = null;
        ...
        public void on_recv(byte[] recv_data)
        {
            ...
            while (true)
            {
                ...
                dynamic unpackedObject = MessagePack.MessagePackSerializer.Deserialize<ProtoRoot>(_tmp_data);
                lock (que)
                {
