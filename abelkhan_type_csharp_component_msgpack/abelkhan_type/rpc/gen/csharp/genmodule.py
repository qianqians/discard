...
def gen_module_module(module_name, funcs, dependent_struct, dependent_enum):
    ...
    for i in funcs:
        ...
        if i[1] == "ntf":
            ...
            code_func += "            var _struct = MessagePackSerializer.Deserialize<" + module_name + "_" + func_name + "_struct_ntf>(_event);\n"
            ...
            code_func += "        public void " + func_name + "(byte[] _event){\n"
            code_func += "            var _struct = MessagePackSerializer.Deserialize<" + module_name + "_" + func_name + "_struct_req>(_event);\n"
            code_func += "            rsp = new rsp_" + func_name + "(current_ch, _struct._cb_uuid);\n"
            code_func += "            if (on" + func_name + " != null){\n"
            code_func += "                on" + func_name + "("
            count = 0
            for _type, _name in i[2]:
            ...
            rsp_code += "            var __byte_struct__ = MessagePackSerializer.Serialize(__argv_struct__);"
