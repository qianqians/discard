...
namespace http_gate
{
    public class utils
    {
        ...
        private static void inputByte(byte value, ref uint crcResult)
        {
            crcResult = (crcResult << 8) ^ crcTable[(crcResult >> 24) ^ value];
        }

        public static byte[] Serialize(string uuid, byte[] body)
        {
            var rsp = new HttpBaseRsp() { uuid = uuid, timeStamp = (ulong)timerservice.Tick, rawData = new byte[2 * body.Length + 4] };

            byte[] xor_key = new byte[8];
            byte tmp_key = (byte)(rsp.timeStamp & 0xff);
            xor_key[0] = (byte)(rsp.uuid[0] ^ tmp_key);
            xor_key[1] = (byte)(rsp.uuid[1] ^ tmp_key);
            xor_key[2] = (byte)(rsp.uuid[2] ^ tmp_key);
            xor_key[3] = (byte)(rsp.uuid[3] ^ tmp_key);
            xor_key[4] = (byte)(rsp.uuid[4] ^ tmp_key);
            xor_key[5] = (byte)(rsp.uuid[5] ^ tmp_key);
            xor_key[6] = (byte)(rsp.uuid[6] ^ tmp_key);
            xor_key[7] = (byte)(rsp.uuid[7] ^ tmp_key);
            ...
            {
                inputByte(body[i], ref crcResult);
                rsp.rawData[j++] = (byte)(body[i] ^ xor_key[i & 0x7]);

                var v = (byte)rnd.Next(0, 256);
                inputByte(v, ref crcResult);
                rsp.rawData[j++] = v;
            }

            byte[] crcBytes = BitConverter.GetBytes(crcResult);
            rsp.rawData[j++] = crcBytes[0];
            rsp.rawData[j++] = crcBytes[1];
            rsp.rawData[j++] = crcBytes[2];
            rsp.rawData[j++] = crcBytes[3];

            return MessagePackSerializer.Serialize(rsp);
        }

        public static HttpBasePacket Deserialize(byte[] body)
        {
            var req = MessagePackSerializer.Deserialize<HttpBasePacket>(body);

            byte[] xor_key = new byte[8];
            byte tmp_key = (byte)(req.timeStamp & 0xff);
            xor_key[0] = (byte)(req.uuid[0] ^ tmp_key);
            xor_key[1] = (byte)(req.uuid[1] ^ tmp_key);
            xor_key[2] = (byte)(req.uuid[2] ^ tmp_key);
            xor_key[3] = (byte)(req.uuid[3] ^ tmp_key);
            xor_key[4] = (byte)(req.uuid[4] ^ tmp_key);
            xor_key[5] = (byte)(req.uuid[5] ^ tmp_key);
            xor_key[6] = (byte)(req.uuid[6] ^ tmp_key);
            xor_key[7] = (byte)(req.uuid[7] ^ tmp_key);

            var _new_body = new byte[(req.rawData.Length - 4) / 2];
            uint crcResult = 0xffffffff;
            int j = 0;
            for(int i = 0; i < req.rawData.Length - 4; i += 2)
            {
                req.rawData[i] ^= xor_key[j & 0x7];
                _new_body[j++] = req.rawData[i];

                inputByte(req.rawData[i], ref crcResult);
                inputByte(req.rawData[i + 1], ref crcResult);
            }

            byte[] crc = BitConverter.GetBytes(crcResult);
            if (crc[0] == req.rawData[req.rawData.Length - 4] &&
                crc[1] == req.rawData[req.rawData.Length - 3] &&
                crc[2] == req.rawData[req.rawData.Length - 2] &&
                crc[3] == req.rawData[req.rawData.Length - 1])
            {
                req.rawData = _new_body;
            }
            else
            {
                throw new AbelkhanException("crc error");
            }
